...
class TonIndexer:
    ...
    def get_account_transactions(self, account, data_limit=100, query_params=None):
        limit = self.config["chunks"]["transactions"]
        if limit > data_limit:
            limit = data_limit

        self.log.log(self.__class__.__name__, 3,
                     "Fetching transactions for account {} with limit {}".format(account, limit))

        if not query_params:
            query_params={}

        query_params["address"] = account
        query_params["limit"] = limit

        data = self.query("getTransactionsByAddress", payload=query_params, use_chunks=True, data_limit=data_limit)
