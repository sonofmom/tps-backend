...
def get_datetime_utc(timestamp=None, return_none=False):
    if not timestamp and return_none:
        return None
    if not timestamp:
...
def parse_runmethod_response(response):
    result = []
        ...
        if isinstance(element, list):
            if element[0] == 'num':
                if element[1][0] == '-':
                    ...
                    if len(hex) % 2:
                        hex = "0"+hex
                    parsed = int.from_bytes(bytearray.fromhex(hex),"big")

            elif element[0] == 'cell':
                parsed = tvm_valuetypes.cell.deserialize_boc(base64.b64decode(element[1]["bytes"].encode('utf-8')))
